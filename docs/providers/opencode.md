...
OpenCode's own logs already on your PC. Nothing is sent anywhere.
...
When you have the Go subscription, AIUsage.NET shows "Go" beside the provider name.

The Session / Weekly / Monthly meters show **observed local spend** — the usage recorded on *this* PC. If
...
word. (When OpenCode ships an official usage API, AIUsage.NET can switch to authoritative numbers without any
...
Use OpenCode as usual. AIUsage.NET reads OpenCode's local data directory
(`%LOCALAPPDATA%\opencode`, or `$OPENCODE_DATA_DIR` / `$XDG_DATA_HOME` if you've set them — the Windows
counterpart of `~/.local/share/opencode`): the `auth.json` Go key to detect that you use it, and the
local SQLite logs (read via Microsoft.Data.Sqlite) for the numbers. There's no login prompt and no
token to paste.
...
usage reads "No data" rather than a misleading `$0.00`. No log data leaves your PC.

The Go caps AIUsage.NET draws against are the published plan limits: **$12 per rolling 5 hours**, **$30 per
...
- **Everything shows "No data"** — AIUsage.NET needs OpenCode's local database at
  `%LOCALAPPDATA%\opencode\opencode*.db`. Run an OpenCode session, then refresh. (If you're logged into
...
- **No Session / Weekly / Monthly meters** — those are Go-plan caps; you'll see them when you're logged
  into OpenCode Go or have used it recently on this PC. Zen-only (or lapsed) users see the spend tiles
...
- **Numbers look lower than your dashboard** — the meters are local-observed spend (this PC only); see the
  note above.

## Under the hood

AIUsage.NET reads the assistant-message `cost` and token fields from every `opencode*.db` in the data
