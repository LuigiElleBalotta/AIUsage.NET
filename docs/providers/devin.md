...
When Devin reports your plan name, AIUsage.NET shows it beside the provider name.

## Where credentials come from

Checked in this order — whichever works first wins:

1. Devin CLI credentials: `%LOCALAPPDATA%\devin\credentials.toml` (the Windows counterpart of `~/.local/share/devin/credentials.toml`; uses `windsurf_api_key`, and `api_server_url` when present)
2. The Devin/Windsurf app's local state database (`%APPDATA%\Devin\User\globalStorage\state.vscdb`), read via Microsoft.Data.Sqlite instead of the macOS `sqlite3` CLI
