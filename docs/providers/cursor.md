...
When Cursor reports your plan name, AIUsage.NET shows it beside the provider name.

## Where credentials come from

Just be signed into the Cursor app. AIUsage.NET reads Cursor's local state database — `%APPDATA%\Cursor\User\globalStorage\state.vscdb`, the same file Cursor itself uses, read via Microsoft.Data.Sqlite instead of a `sqlite3` CLI — for the session tokens, with a Windows Credential Manager fallback. Refreshed tokens are persisted back to whichever source they came from. Nothing extra to install or configure.

## Spend history

Today, Yesterday, Last 30 Days, and Usage Trend come from Cursor's usage export. AIUsage.NET uses the exported token counts and shared model pricing to estimate the cost locally. Cursor's export may occasionally arrive late, so the newest figures can lag behind current activity. AIUsage.NET leaves isolated malformed rows out instead of silently counting broken values as zero. A failed download, invalid export schema, or broken CSV structure leaves spend history unavailable for that refresh. Each failure is recorded in the diagnostic log without including the exported usage data.
...
- **Optional lookup failed** — plan, credit-grant, prepaid-balance, and request-fallback failures stay nonfatal when primary usage is available. AIUsage.NET records fixed, credential-free reasons in the diagnostic log.
