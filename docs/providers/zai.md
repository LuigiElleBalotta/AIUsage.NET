...
When Z.ai reports your plan name, AIUsage.NET shows it beside the provider name.

## Where credentials come from

Z.ai has no companion CLI/app that AIUsage.NET can reuse a credential from, so you supply an API key.
AIUsage.NET reads it from the first place it finds one, in this order:

1. `~/.aiusage/zai.json` — `{"apiKey":"…"}` (the file Settings writes to)
...
way, nothing leaves your PC except the same API calls Z.ai's own subscription UI makes.
...
2. Add the key to AIUsage.NET via **Settings → API Keys**, **or** set it as an environment variable:

```powershell
setx ZAI_API_KEY "YOUR_API_KEY"
```

3. Z.ai appears on the dashboard on the next refresh.
...
- **"No Z.ai API key"** — add a key in Settings → API Keys, or set `ZAI_API_KEY`.
