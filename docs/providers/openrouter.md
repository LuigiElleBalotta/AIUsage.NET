...
AIUsage.NET shows the reported tier (such as "Pay as you go" or "Free tier") beside the provider name.
...
The key is stored at `~/.aiusage/openrouter.json` and picked up on the next refresh.

You can also provide the key directly (checked in this order, first match wins):

1. **Config file:** `~/.aiusage/openrouter.json` — the file the Settings card writes:
...
2. **Environment variable:** set `OPENROUTER_API_KEY` in your environment (User environment variables,
   or your shell profile). A key exported there is picked up even when the app is started from the
   Start menu, not just when run from a terminal. When a key is found here, the API Keys card shows it
   as read-only ("From environment") with a checkbox to override it with a saved key.
