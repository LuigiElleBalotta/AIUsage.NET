...
| Kiro | `[DISABILITATO — 0.4.1]` codice e test presenti ma **non registrato in `ProviderCatalog.Make()`** — nessuna installazione lo esegue | `[NUOVO, ora sospeso]` Provider non portato da OpenUsage/Swift (Kiro è un IDE AWS, non esisteva nel progetto originale). Reverse-engineered da `Kiro-Go`/`kiro-gateway` (proxy open source di terze parti) invece che dalla documentazione ufficiale (inesistente per l'usage API). Due fonti di credenziali locali: file JSON Kiro IDE (`~/.aws/sso/cache/kiro-auth-token.json`, che riusa la cartella cache SSO ma non è una vera voce SSO) e il DB SQLite di `kiro-cli` (`%LOCALAPPDATA%\Kiro-Cli\data.sqlite3` su Windows, via `Microsoft.Data.Sqlite` come Cursor/Devin). Endpoint `GET .../getUsageLimits` su `codewhisperer.us-east-1.amazonaws.com` o `q.{region}.amazonaws.com` a seconda della region del profilo CodeWhisperer risolto (mai la region SSO/auth, che può non avere un host dati corrispondente — bug verificato e corretto contro l'account reale usato per il test). Traccia anche l'overage pay-as-you-go (`overageConfiguration`/`overageCap`/`overageRate`/`currentOverages`, stessi campi della risposta base). 15 test unitari sul mapper.
...
`[DISABILITATO — 0.4.1]` **Terzo incidente, dopo entrambi i fix sopra: provider disattivato del tutto per tutti.** Nonostante il fix del refresh proattivo (0.3.1) e quello del retry del token morto (0.4.0), l'utente ha segnalato un nuovo logout forzato da Kiro dopo un test della build 0.4.0 — proprio subito dopo il lancio manuale dell'exe pubblicato. La causa esatta di questo terzo episodio non è stata identificata (possibili ipotesi: un terzo percorso di codice che tocca ancora il refresh, come la risoluzione del profile ARN; un'interazione fra le due fonti di credenziali — file desktop e DB SQLite di `kiro-cli` — quando entrambe sono presenti; oppure una finestra di reuse-detection lato AWS più aggressiva di quanto assunto). Decisione: **non tentare un quarto fix incrementale contro l'account reale dell'utente**. Rimossa la riga `providers.Add(new KiroProvider())` da `ProviderCatalog.Make()` (`src/AIUsage.Core/Providers/ProviderCatalog.cs`) — nessuna registrazione, quindi il provider non compare in `WidgetRegistry`, non viene mai eseguito dal refresh loop, e resta invisibile in UI anche per chi l'aveva già abilitato in `enabledProviders.v1` (il suo id semplicemente non fa più match con nulla). Codice e test (`Providers/Kiro/*.cs`, `KiroProviderTests.cs`, `KiroUsageMapperTests.cs`) lasciati intatti e compilano/passano — riattivarlo è una riga sola una volta risolto. Tracciato in [issue #8](https://github.com/LuigiElleBalotta/AIUsage.NET/issues/8).
